...
def dp_pixelate(img, target_h, target_w, m, eps, noise_func="laplace",
                noise_factor = 1, 
    ...
    for i in range(num_channels):
      if (noise_func=="laplace") or (noise_func=="l"):
        dp_px_img[:,:,i] = Noise.add_laplace_noise(px_img[:,:,i], 0, scale, noise_factor=noise_factor)

      elif (noise_func=="gaussian") or (noise_func=="g"):
        dp_px_img[:,:,i] = Noise.add_gaussian_noise(px_img[:,:,i], 0, 1, noise_factor=noise_factor)

      elif (noise_func=="exponential") or (noise_func=="e"):
        dp_px_img[:,:,i] = Noise.add_exponential_noise(px_img[:,:,i], scale, noise_factor=noise_factor)
      
      elif (noise_func=="uniform") or (noise_func=="u"):
        dp_px_img[:,:,i] = Noise.add_uniform_noise(px_img[:,:,i], low=0, high=1, noise_factor=noise_factor)
      
      else:
        print(" please select in laplace, gaussian, exponential, uniform noise")
